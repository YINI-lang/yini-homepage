...
- (c.) **Numeric shorthand section markers:**  
  To create deeply nested sections at level 10 or deeper, use numeric shorthand:

    ```yini
    ^9 DeepSection    # Equivalent to 9 carets: ^^^^^^^^^ DeepSection
    <10 VeryDeep      # Level 10; repeated marker form would be invalid.
    ```
