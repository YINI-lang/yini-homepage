...
description: "Learn the YINI configuration format in a practical 10–15-minute guide covering syntax, sections, values, nesting, parser modes, and examples."
---
...
**A Practical Guide to YINI Syntax and Structure**

Go beyond the basics in 10–15 minutes with a step-by-step look at YINI
syntax, nested structure, values, parser modes, and practical features.

**New to YINI?** Start with the
[YINI 101: Quick Tutorial](/learn-yini/quick-tutorial).

**Prefer examples?** See [real YINI configs](/learn-yini/examples), or try
the syntax directly in the [YINI Playground](/playground).
...
Values can be different types, such as strings, numbers, booleans, null, lists (arrays), and inline objects:
...
For deeper repeated section markers, underscores can visually group the
markers and make the section depth easier to count. The underscores do not
change the depth:

```yini
^^^^^^ Settings   # Level 6
^^^_^^^ Settings  # Also level 6
```
