...
YINI supports nested sections, lists (arrays), inline objects, and comments. It can store app or service settings in files such as `config.yini`.
...
- Flexible strings using single (`'`) or double (`"`) quotes: raw strings keep text such as
  file paths readable, `C`-prefixed strings support escape sequences, and
  triple-quoted (`"""`) strings handle multi-line text.
- Lists (arrays) `[1, 2, 3]` and inline objects `{ key: "value" }`.
...
- **Clear nested structure** – Supports nested sections, lists (arrays), and inline objects without using indentation to define structure.
...
The interpretation **YAML + INI** is understandable, but that is not where the name came from. YINI is inspired mainly by the simplicity of INI-style configuration files, while adding structured features such as nested sections, lists (arrays), inline objects, and strict/lenient parsing.
...
- **INI** has no single universal standard for nested structure, lists (arrays), or typed values. In basic INI-style files, values are often treated as strings, and each tool may handle more complex structures differently.
...
- **Clear structure** — real sections, lists (arrays), and objects, without relying on indentation to define nesting.
...
Lenient mode is the default. It accepts some minor issues, such as trailing commas in lists (arrays) or inline objects, while still reporting warnings and errors.
...
- **Lists (arrays):** `[1, 2, "str"]`
- **Inline objects:** `{ host: "db", port: 5432 }`

### Q – Does YINI support lists (arrays) and inline objects?
