---
title: 'Get Started: YINI'
description: Create your first .yini data file, validate it, and learn the core syntax in minutes.
---

import CONFIG from '../config/conf'
import BaseLayout from '../layouts/BaseLayout.astro'

<BaseLayout headTitle={frontmatter.title} headDescription={frontmatter.description}>

# Get Started with YINI

Welcome! This guide helps you create your **first working `.yini` file (text-based)** in just a few minutes. Learn how to install and use YINI from the terminal or command line (CLI).  

YINI is a human-friendly data format — like INI, but with nested sections, comments, lists (arrays), and objects. It is suitable for configuration files, application settings, and general data-storage use cases.

## Prerequisite: Node.js installed

- The examples assume you have Node.js installed on your system.

💡 Tip: You can check if you have Node installed with:
```bash
node -v
```

It's recommended to use **Node.js 16 or later**.

---

## 1) Install yini-cli (or just use npx)

**Option A — use without installing**

```bash
npx yini-cli --help
```

**Option B — enable yini CLI globally**

```bash
npm i -g yini-cli
```

**Option C — add the CLI to your Node.js project**

```bash
npm i -D yini-cli
```

> After installing, you can run npx yini-cli or add an npm script like "yini": "yini".

💡 You can use either **npx yini** (no install), or just **yini** if `yini-cli` is installed globally or locally.

## 2) Create your first .yini file

Save this as `config2.yini`:

```yini
^ App
  name = "Demo"
  version = "1.0.0"
  features = [ "search", "dark-mode" ]  // Comments are allowed

^ Database
  host = "localhost"
  port = 5432
  auth = { user: "admin", pass: "secret" }  // Object literal
```

**Notes:**

- **Sections:** start with `^`. Add more carets for nesting (sub-sections): (`^`, `^^`, `^^^`, ...).
- **Members:** written as `key = value` pairs.
- **Comments:** Use `//` or `#` (including a space after the hash) for inline comments.
- **Arrays:** `[1, 2, 'three']`
- **Objects:** `{ key: 'value', n: 1 }`
- **Strings:** must be quoted `'like this'` or `"like this"`.
- For more details, see: [Intro to the YINI format](/intro-yini-config-format).


## 3) Parse your `.yini` file

Once your file is saved, parse it with the CLI:

If you used **option A** (with npx) in step 1, type the following:
```bash
npx yini-cli parse config2.yini
```

Or, if you used **option B or C** in step 1:
```bash
yini parse config2.yini
```

#### JS Output:

You should see output like (JavaScript Object):
```yini
{
  App: {
    name: 'Demo',
    version: '1.0.0',
    features: [ 'search', 'dark-mode' ]
  },
  Database: {
    host: 'localhost',
    port: 5432,
    auth: { user: 'admin', pass: 'secret' }
  }
}
```

## 4) Other output styles

### 4.1) Results in JSON

If you want the **output in pure JSON**, use the `--json` flag.

If you used **option A** (with npx) in step 1, type the following:
```bash
npx yini-cli parse --json config2.yini
```

Or, if you used **option B or C** in step 1:
```bash
yini parse --json config2.yini
```

#### JSON Output:
```json
{"App":{"name":"Demo","version":"1.0.0","features":["search","dark-mode"]},"Database":{"host":"localhost","port":5432,"auth":{"user":"admin","pass":"secret"}}}
```

### 4.2) Results in pretty JSON

If you want the **output in pretty JSON** so it's easier to read, use the `--pretty` flag.

If you used **option A** (with npx) in step 1, type the following:
```bash
npx yini-cli parse --pretty config2.yini
```

Or, if you used **option B or C** in step 1:
```bash
yini parse --pretty config2.yini
```

#### Pretty JSON Output:
```json
{
    "App": {
        "name": "Demo",
        "version": "1.0.0",
        "features": [
            "search",
            "dark-mode"
        ]
    },
    "Database": {
        "host": "localhost",
        "port": 5432,
        "auth": {
            "user": "admin",
            "pass": "secret"
        }
    }
}
```

💡 Common tips:
- Keys and section names are case-sensitive.
- Strings must always be quoted, either `'` or `"`.

## Summary
✅ You have just created and parsed your first YINI configuration file, into both JavaScript and JSON formats!

---

## 🧭 Next Steps

- ▶️ [Quick Tutorial](/yini-tutorial) — 5-minute guided walkthrough of YINI.  
- 📖 [Intro to YINI format](/intro-yini-config-format) — 10–15 minute introduction to YINI format's syntax and structure.
- 📘 [Specification](/specification) — Read the full YINI Specification.  

</BaseLayout>
