...
- 🌐 **Browser playground** - try YINI without installing anything.
...
Use the [YINI Playground](/playground) if you want to experiment in the
browser first.
This guide focuses on the **CLI**, which is the best starting point for local files and automation.
...
- **Comments:** Use `//` or `#` for inline comments; no whitespace is required after `#`.
- **Arrays:** `[1, 2, 'three']`
